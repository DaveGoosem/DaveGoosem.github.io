...
const PreviewSearchComponent = ({ rfkId }: PreviewSearchProps) => {
  ...
  const {
    ...
    query: (query) =>
      query
        .getRequest()
        .setSearchQueryHighlightFragmentSize(100),
  });
...
export const PreviewSearch = widget(PreviewSearchComponent, PreviewSearchUI.Default, 'preview-search');
```
  ...
  const {
    ...
    query: (query) => {
      query
        ...
        .addSearchQueryFilter(new FilterEqual('site_name', process.env.NEXT_PUBLIC_SITE_NAME as string));
    },
  });

  return (
    <div ref={widgetRef}>
      <p>{totalItems} results for "{keyphrase}"</p>
