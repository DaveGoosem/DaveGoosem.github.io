...
In a headless solution, especially a SaaS hosted one, the way in which you need to look at what has been historically a server-side process needs to be re-considered.
...
We are going to focus on the solution design concepts for the Head application only and how to make the content of the PDF/Email dynamic and not on the actual generation itself which we'll get into 'why' shortly.
